...
const Reports: React.FC<ReportsProps> = ({ user }) => {
  ...
  const downloadCSV = () => {
    ...
    setTimeout(() => {
      ...
      link.setAttribute("download", `${APP_FILE_PREFIX}_${reportType}_Export.csv`);
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      doc.save(`${APP_FILE_PREFIX}_${reportType}_Export.pdf`);
    } catch (err: any) {
  ...
  const downloadSummaryCSV = () => {
    ...
    setTimeout(() => {
      ...
      const link = document.createElement('a');
      link.setAttribute('href', encodeURI(csvContent));
      link.setAttribute('download', `${APP_FILE_PREFIX}_Employee_Summary_${startDate}_to_${endDate}.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setIsGenerating(false);
  ...
  const downloadSummaryPDF = async () => {
    ...
    try {
      ...
      doc.save(`${APP_FILE_PREFIX}_Employee_Summary_${startDate}_to_${endDate}.pdf`);
    } catch (err: any) {
