...
import { APP_NAME, APP_FILE_PREFIX } from '../constants';
...
const EmployeeDirectory: React.FC<EmployeeDirectoryProps> = ({ user, selectedEmployeeId }) => {
  ...
  const getExportFilename = (ext: string) => {
    if (selectedExportDepts.length === 0 || selectedExportDepts.length === depts.length) return `${APP_FILE_PREFIX}_Employee_Directory.${ext}`;
    if (selectedExportDepts.length === 1) return `${APP_FILE_PREFIX}_${selectedExportDepts[0].replace(/\s+/g, '_')}_Directory.${ext}`;
    return `${APP_FILE_PREFIX}_${selectedExportDepts.length}_Departments_Directory.${ext}`;
  };
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(`Generated by ${APP_NAME} on ${now}`, 14, pageHeight - 8);
